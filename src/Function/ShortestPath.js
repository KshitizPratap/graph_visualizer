export function BFSPathfinder(grid, visited, startCoordinate, endCoordinate) {
  let animated = [];
  let visi = [];

  for (var i = 0; i < visited.length; i++) visi[i] = visited[i].slice();
  ...
  while (i < queue.length && queue.length > 0) {
    ...
    for (let j = 0; j < 4; j++) {
      if (
        ...
        if (
          curr[0] + row[j] === endCoordinate[0] &&
          curr[1] + col[j] === endCoordinate[1]
        ) {
          animated.push(curr);
