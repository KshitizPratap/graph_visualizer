    ...
    for (let i = rangeRow; i - rangeRow < range && i < row - 1; i++) {
      for (let j = rangeCol; j < col - 1; j++) {
        if (
          ...
          (i - rangeRow == 3 && i<row-2 && j - rangeCol > 2 && j < col-2) ||
          (i > rangeRow+3 && i<row-2 && i-rangeRow<range-1 && j - rangeCol === 4 && j<col-2)
        ) {
    ...
    if (row - rangeRow > rangeRow) {  
      for(let j=rangeCol; j-rangeCol<=range && j<col-1; j++){
          for(let i=1; i<=rangeRow && i<row-1; i++){
              if((i === rangeRow-2 && i>3 && j-rangeCol <= range) || 
                (i === rangeRow-4 && i>3 && j-rangeCol <= range && j>rangeCol) ||
                (i<rangeRow-4 && i>3 && j === rangeCol+1 && j<col-2) ||
                (i<rangeRow-4 && i>1 && j === rangeCol+3 && j<range+rangeCol && j<col-2) ||
      ...
      for(let j=rangeCol; j-rangeCol<=range && j<col-1; j++){
        for(let i=rangeRow; i<row-1; i++){
            if((i === rangeRow+2 && i<row-2 && j-rangeCol <= range) ||
                (i === rangeRow+4 && i<row-2 && j-rangeCol < range && j>rangeCol) ||
                (i >= rangeRow+4 && j === rangeCol+1 && j !== col-4 && j<col-2) ||
                (i === rangeRow+6 && i<row-2 && j-rangeCol <= range) ||
                (i>=rangeRow+6 && (j === col-3 || (j-rangeCol === range-1 && j!==col-4)) && j<col-2)
                ){
                  visi[i][j] = true;
              }
              else
